...
public final class MyStrategy implements Strategy {
    ...
    private static final double ENEMY_TO_CLOSE_FACTOR = 0.5D;
    private static final double ENEMY_TO_FAR_FACTOR = 0.9D;
...
//        move.setStrafeSpeed(random.nextBoolean() ? game.getWizardStrafeSpeed() : -game.getWizardStrafeSpeed());
        ...
        // Выбираем цель
        final LivingUnit bestTarget = getBestTarget();
        final LivingUnit nearestTarget = getNearestTarget();
    ...
    private LivingUnit getBestTarget() {
        final List<LivingUnit> potentialNotNeutralTargets = getPotentialNotNeutralTargets();
        final List<LivingUnit> wizards = potentialNotNeutralTargets.stream().filter(unit -> unit instanceof Wizard).collect(Collectors.toList());
        if(wizards.size() > 0){
            return getMinimalHpTarget(wizards);
        } else{
            return getMinimalHpTarget(potentialNotNeutralTargets);
        }
    }
     ...
     * Находим ближайшую цель для атаки, независимо от её типа и других характеристик.
     */
    private LivingUnit getNearestTarget(List<LivingUnit> targets) {
    ...
    /**
     * Получает список врагов, доступных для атаки
     */
    private List<LivingUnit> getPotentialNotNeutralTargets() {
        final List<LivingUnit> livingUnits = new ArrayList<>();
        ...
        return livingUnits
                .stream()
                .filter(unit -> !unit.getFaction().equals(Faction.NEUTRAL))
                .filter(unit -> inDifferentFactions(self, unit))
                .filter(unit -> self.getDistanceTo(unit) <= self.getCastRange())
                .collect(Collectors.toList());
    }

    /**
     * Определяет являются ли юниты врагами
     */
    private boolean inDifferentFactions(LivingUnit unit1, LivingUnit unit2) {
        return !unit1.getFaction().equals(unit2.getFaction());
    }

    /**
     * Выбираем юнита с наименьшим ХП
     */
    private LivingUnit getMinimalHpTarget(List<LivingUnit> livingUnits) {

        if (livingUnits.size() <= 0) {
            return null;
        }

        LivingUnit lowHpUnit = livingUnits.get(0);

        for (LivingUnit unit : livingUnits) {
            if (lowHpUnit.getLife() > unit.getLife()) {
                lowHpUnit = unit;
            }
        }
